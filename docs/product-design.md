...
天气雷达默认关闭。开启成功后，地图左下角展示紧凑图例、雷达帧时间和 RainViewer 署名。雷达图层位于底图之上、机场、航迹和航班之下。最新帧不超过 15 分钟时标记为「最新」；15 分钟至 2 小时标记为「数据延迟」；2 至 24 小时标记为「历史缓存」，并明确说明不是当前天气；超过 24 小时停止展示。首期不提供天气时间轴或历史查询。
