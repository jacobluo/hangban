...
PC 端点击实时状态入口后进入独立整页，展示总体健康状态、数据源结果、最后成功时间和覆盖边界；手机端使用全高、可滚动的状态页面。返回地图时保留地图视野、筛选和选中对象。具体修复边界见 [`superpowers/specs/2026-07-13-desktop-data-status-page-design.md`](superpowers/specs/2026-07-13-desktop-data-status-page-design.md)。

打开状态页时主动检查天气雷达状态，即使地图雷达图层仍处于关闭状态。天气数据使用独立卡片展示 RainViewer、雷达帧时间、新鲜度、有效性说明和来源署名。天气异常不改变航班总体健康状态，也不进入「正常来源数」统计。具体边界见 [`superpowers/specs/2026-07-13-weather-data-status-design.md`](superpowers/specs/2026-07-13-weather-data-status-design.md)。
...
首期筛选用于减少地图噪声，包含高度范围、航空公司、航班状态或数据质量等可由统一模型可靠支持的条件。图层控制用于切换航班、机场、航迹、天气雷达和地图信息层。

天气雷达服务端能力默认启用，前端地图图层默认关闭；图层开关只控制是否在地图上展示雷达，不控制服务端是否接入天气数据。开启图层成功后，地图左下角展示紧凑图例、雷达帧时间和 RainViewer 署名。雷达图层位于底图之上、机场、航迹和航班之下。最新帧不超过 15 分钟时标记为「最新」；15 分钟至 2 小时标记为「数据延迟」；2 至 24 小时标记为「历史缓存」，并明确说明不是当前天气；超过 24 小时停止展示。首期不提供天气时间轴或历史查询。
...
- 时间统一以带时区的 ISO 8601 UTC 传输和保存；当前观测时间由浏览器按系统时区展示，并明确标出 `GMT±N`。原始 UTC 保留在语义化 `<time>` 和悬浮提示中，便于跨时区核对。
...
数据状态反映数据源最近一次实际请求结果，而不是仅根据页面是否仍有航班判断服务正常。状态页应区分：
...
| 天气雷达   | 默认关闭；启用后显示雷达栅格、紧凑图例、帧时间和 RainViewer 署名   | 按 `main_deep（16:1）` 已确认状态实现    |
| 时间展示   | 当前观测时间按浏览器时区显示 `GMT±N`，详情与状态页保留完整日期时间 | 服务端、缓存和时间计算继续使用 UTC       |
...
本轮以 Ardot `Deep / Desktop / Data Coverage and Status（16:1970）` 和 `Deep / Mobile / Data Coverage and Status（16:2032）` 为确认基线，修复 PC 实时状态入口误用 `360 px` 右侧栏的问题。核对截图保存在：

- `.ardot-qa/data-status-page/implementation-desktop.png`
- `.ardot-qa/data-status-page/implementation-mobile.png`

| 核对项     | 核对结果                                                                          | 有意差异或说明                                            |
| ---------- | --------------------------------------------------------------------------------- | --------------------------------------------------------- |
| PC 结构    | 状态页占据完整应用内容区，保留全局顶栏；摘要、数据源和覆盖边界使用整页双栏布局    | 不新增 URL 路由，返回后保留地图视野、筛选和选中对象       |
| 手机结构   | `390 × 844` 下使用全高单列页面，允许纵向滚动，`scrollWidth` 与视口宽度均为 390 px | 不与地图控制或半高详情抽屉同时显示                        |
| 可访问性   | 打开后聚焦「返回地图」；支持 `Escape`；返回后恢复状态入口焦点                     | 手机返回操作高度不小于 44 px                              |
| 数据真实性 | 只展示正常来源数、当前已获得航班数、最后成功时间和现有来源字段                    | 不复刻 Ardot 中无法验证的平均延迟、全球航班数和区域覆盖率 |
| 空来源状态 | 未获得来源状态时显示「尚未获得来源状态」，不把 `0 / 0` 误报为「全部服务正常」     | 当前航班数仍明确不代表全球实际在途总数                    |

完整门禁结果：303 个单元测试、9 个集成测试、19 个 E2E 通过，1 个项目条件测试跳过；lint、typecheck、build 和响应式状态页专项 E2E 均通过。

### 2026-07-13 天气雷达图层

天气雷达按 Ardot `main_deep（16:1）` 的桌面端与手机端确认状态实现。图层默认关闭，用户启用后显示位于航班、机场和航迹下方的雷达栅格，以及包含帧时间、强度色标和 RainViewer 署名的紧凑图例。服务端只短期缓存最近 24 小时内实际请求过的元数据和瓦片；该缓存不构成长期天气归档，也不提供历史查询。

历史缓存状态明确显示「非当前天气」，不可用时自动关闭雷达开关并提示「航班数据不受影响」。视觉证据保存在：

- `.ardot-qa/weather-radar/implementation-desktop.png`
- `.ardot-qa/weather-radar/implementation-mobile.png`

完整门禁结果：398 个单元与组件测试、9 个真实中间件集成测试、25 个 E2E 通过，1 个项目条件测试跳过；lint、typecheck、build、天气雷达专项 E2E 和双视口视觉核对均通过。

### 2026-07-13 天气数据状态

本轮在 Ardot `main_deep（16:1）` 中更新桌面端 `16:1970` 和手机端 `16:2032`，并增加最新、历史与不可用三种状态的天气数据卡片基线。打开「数据覆盖与服务状态」时会独立请求天气雷达状态，即使地图图层仍关闭；状态页展示 RainViewer、雷达帧时间、新鲜度、当前天气有效性和来源署名。

天气请求失败或只有历史缓存时，不改变航班总体健康状态和「正常来源」统计。地图雷达未启用时，状态页检查也不会显示地图级不可用提示或自动打开图层。实现核对截图保存在：

- `.ardot-qa/weather-data-status/implementation-desktop.png`
- `.ardot-qa/weather-data-status/implementation-mobile.png`

完整门禁结果：412 个单元与组件测试、9 个真实中间件集成测试、27 个 E2E 通过，1 个项目条件测试跳过；lint、typecheck、build、格式检查和双视口视觉核对均通过。

### 2026-07-14 浏览器本地时间

本轮更新 Ardot `main_deep（16:1）` 中全球实时、航班详情、航线、天气雷达、状态矩阵和天气数据卡片的时间示例。地图面板、图例和短时回看使用紧凑时间；完整航班详情和数据状态页使用包含日期与秒的完整时间。

前端通过统一 `BrowserTime` 组件读取浏览器时区并显示 `GMT±N`。SSR 与首次 hydration 使用稳定占位，挂载后再显示本地时间；`dateTime` 和悬浮提示保留原始 UTC。排序、新鲜度、天气缓存和航迹回看计算仍使用 UTC 时间戳。

### 2026-07-14 地图交互细节优化

本轮先在 Ardot `main_deep（16:1）` 更新确认状态，再同步当前实现：选中机场使用橙色光环、白色描边核心和始终可见的橙色代码标签；航线机场选择器打开时可越过探索面板边界，选项列表在自身区域滚动；地图图例的可见标题改为「天气雷达降水图」，图层开关和可访问名称继续使用「天气雷达」；从机场周边航班进入航班详情后提供「返回 PEK 周边航班」来源动作，右上角关闭也恢复同一机场上下文。

已使用 1440 × 900 和 390 × 844 视口核对选择器边界、雷达图例和机场来源返回动作。实现证据保存在：

- `.ardot-qa/2026-07-14-map-interaction-polish/desktop-implementation.png`
- `.ardot-qa/2026-07-14-map-interaction-polish/mobile-implementation.png`

轨迹回看仍使用现有 0–15 分钟估算位置，不延长时间范围，也不增加自动回放。

完整门禁结果：430 个单元与组件测试、9 个真实中间件集成测试、31 个 E2E 通过，1 个桌面项目条件用例跳过；lint、typecheck、production build、格式检查和双视口视觉核对均通过。
